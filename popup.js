...
function formatModel(status) {
  ...
  if (status.engine === "artcnn") return String(status.model).replace("ArtCNN_", "ArtCNN ");
...
export function createPopupController({
  ...
  function failed(status) {
    ...
    $("s-webgpu").className = "v no";
    const detail = typeof status.reason === "string" && status.reason ? ` ${status.reason}` : "";
    feedback(`${describeCommandFailure(status)}${detail}`, "error", "context");
    updateAvailability();
  }

  function runtimeMessage(status) {
    if (status.gpuRecovering) return "Recovering the WebGPU device…";
    if (status.documentSuspended) return "Rendering is suspended while this page is hidden.";
    if (status.interpFailure) {
      const detail = status.interpFailure.detail ? `: ${status.interpFailure.detail}` : ".";
      return `Interpolation stopped after repeated failures${detail}`;
    }
    if (status.interpPausedByNeural) return "Frame interpolation is paused while the neural upscaler is selected.";
    if (status.mode !== "off" && status.activeMode === "off") {
  ...
  function render(status) {
    ...
    setBooleanStatus($("s-webgpu"), status.webgpu === true);
    setBooleanStatus($("s-video"), status.hasVideo === true);
    $("s-model").textContent = formatModel(status);
    $("s-model").className = `v ${status.model || status.neuralModel || status.neural ? "ok" : ""}`;
