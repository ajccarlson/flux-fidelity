...
The current extension package still contains the unidentified RIFE model, the SPAN smoke model, the unresolved deband port, and material covered by the LGPL and ONNX Runtime review gates. The removed FSRCNNX assets are absent from both the current tree and package, but their private-history tombstones still block repository publication. Therefore, a successful `npm run check` or `npm run package:internal` does **not** make either distribution surface publicly releasable; `npm run package` enforces the blocking public-release check.
...
`npm run check:generated` regenerates the standard x2 and three ArtCNN pairs and requires byte-for-byte equality. That proves local reproducibility from the checked-in source bytes; it does not by itself establish ownership or permission.
